...
namespace gta_ns{
    ...
    class TrackingNodelet : public nodelet::Nodelet
    {
        ...
        private:
            ...
            Vector3f state_true;
            Vector3f meas_pos;
            Vector3f pre_meas_pos;
            int notFoundCount;
            bool kf_init = false;
            Vec4 pc_meas_pos;
            Vec3 tra_pc_meas_pos;
            bool measured = false;

            // Sensors fusion processing variable
            ...
            message_filters::Subscriber<livox_ros_driver::CustomMsg> sub_pc_syn;
            ...
            // callback
            void imgsyn_cb(const sensor_msgs::CompressedImage::ConstPtr &rgbimage, 
            ...
            virtual void onInit()
            {
                ...
                // subscriber
                sub_img_syn.subscribe(nh, "/camera/color/image_raw/compressed", 100);
                sub_pc_syn.subscribe(nh, "/livox/lidar", 100);
                sync = new message_filters::Synchronizer<MySyncPolicy>(MySyncPolicy(1000), sub_img_syn, sub_pc_syn);
                sync->registerCallback(boost::bind(&TrackingNodelet::imgsyn_cb, this, _1, _2));

                // publisher
                pub_ver = nh.advertise<gta::vertice>("/obj_box_vertices", 1);
                pub_state = nh.advertise<gta::obj>("/obj_pos_body", 1);
        }
    };
