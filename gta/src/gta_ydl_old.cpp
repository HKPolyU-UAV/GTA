    ...
    if (!frame.empty())
    {
        ...
        if (temp.size() != 0)
        {
            ...
            if (potential.size() != 0)
            {
                ...
                int depthbox_w = potential[maxElementIndex].rect.width * 0.45;
                int depthbox_h = potential[maxElementIndex].rect.height * 0.45;
...
void gta_ns::TrackingNodelet::pcl_cb(const livox_ros_driver::CustomMsg::ConstPtr &pclmsg)
{
    ...
    if (kf_init)
    {
        ...
        {
            float a, b, c;
            a = pre_meas_pos[0] - meas_pos[0];
            b = pre_meas_pos[1] - meas_pos[1];
            c = pre_meas_pos[2] - meas_pos[2];
    ...
    if (!kf_init)
    {
        if (yolo_in)
        {
            ...
            if (depth_pcl->points.size() < 5)
            {
                ...
                meas_pos[1] = img_meas_pos[1];
                meas_pos[2] = img_meas_pos[2];
                // cout << "kf_init img_meas_pos: " << meas_pos << endl;
                use_dpeth = true;
                if (meas_pos[0] != 0)
                {
                    measured = true;
            ...
            else
            {
                ...
                meas_pos[1] = pc_meas_pos[1];
                meas_pos[2] = pc_meas_pos[2];
                // cout << "kf_init pc_meas_pos: " << meas_pos << endl;
                use_dpeth = false;
                if (meas_pos[0] != 0)
                {
                    measured = true;
...
void gta_ns::TrackingNodelet::PTfilter(pcl::PointCloud<pcl::PointXYZ>::Ptr pcl_track, double cluster_size)
{
    ...
    // for(int i = 0; i<11; i++)
    // {
    //     cout<<"after PT pointcloud sample: "<< pcl_track->points[i+50]<<endl;
    // }
}
